-- DB - from G:\SteamLibrary\steamapps\common\Sid Meier's Civilization VI\DLC\Expansion2\UI\Replacements\DiplomacyDealView.lua
...
-- DB		2021-03-12: line 445
		if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and ms_OtherPlayerIsHuman and DealDirection.OUTGOING then
			bDealValid = true;
		end
-- /DB
...
-- DB		2021-03-30: line 776 [BM_RFI: disable right increment arrow]
					if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") then
						Controls.ValueAmountEditRightButton:RegisterCallback( Mouse.eLClick, function() OnValueAmountEditDelta(itemID, 0); end );
					else
...
-- DB		2021-03-30: line 866 [BM_RFI: disable click-increments for the other player]
					if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
						return ;
					end
-- /DB
...
-- DB		2021-03-30: line 894 [BM_RFI: only add 1 to the other player's deal]
				if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
					iAddAmount = 1
				end
-- /DB
...
-- DB		2021-03-30: line 942 [BM_RFI: disable click-increments for the other player]
					if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
						return ;
					end
-- /DB
		...
		if (not bFound) then
			...
			if (pDealItem ~= nil) then
				...
				-- Adjust the gold to our max
-- DB		2021-03-30: line 970 [BM_RFI: only add 1 to the other player's deal]
				if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
					iAddAmount = 1
				end
-- /DB
				...
				if (not ms_OtherPlayerIsHuman) then
					...
					if (ms_LastIncomingDealProposalAction == DealProposalAction.PENDING) then
						...
						if (itemsFromLocal > 0 and itemsFromOther == 0) then
							...
							Controls.AcceptDeal:LocalizeAndSetText("LOC_DIPLOMACY_DEAL_GIFT_DEAL");
-- DB		2021-03-30: line 1135 [BM_RFI: control buttons and panels when local player hasn't offered anything]
						elseif GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and (itemsFromLocal == 0 and itemsFromOther == 0) then
							g_uiMyOffers.Directions:SetHide(true);
							g_uiTheirOffers.Directions:SetHide(true);
							Controls.AcceptDeal:SetHide(true);
						elseif GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and (itemsFromLocal == 0 and itemsFromOther > 0) then
							g_uiMyOffers.Directions:SetHide(false);
							g_uiTheirOffers.Directions:SetHide(true);
							Controls.AcceptDeal:SetHide(true);
-- /DB
						else
...
-- DB		2021-03-30: line 1204 [BM_RFI: Human opponent doesn't tell you when a deal is "invalid"]
			if (GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and not ms_OtherPlayerIsHuman) or (not GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO")) then
...
-- DB		2021-03-30: line 1218 [BM_RFI: Hide "Accept Deal" button when the deal isn't valid]
		if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and (not bDealValid) then
			Controls.AcceptDeal:SetHide(true);
		end 
-- /DB
		if (ms_bIsDemand) then
...
-- DB		2021-03-30: line 1297 [BM_RFI: Hide available gold]
					if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") then
...
-- DB		2021-03-30: line 1731 [BM_RFI: clip values only for self, not opponent]
			if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") and pDealItem:GetFromPlayerID() == g_LocalPlayer:GetID() then
...
-- DB		2021-03-30: line 1785 [BM_RFI: Hide amount]
					if GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") then
						uiIcon.AmountText:SetHide(true);
					else
						uiIcon.AmountText:SetHide(false);
...
-- DB		2021-03-30: line 2009 [BM_RFI: Hide city tooltips]
...
-- DB		2021-03-30: line 2698 [BM_RFI: hide city tooltips]
					if not GameConfiguration.GetValue("BM_REMOVE_FOREIGN_INFO") then
